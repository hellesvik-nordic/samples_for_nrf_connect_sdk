...
## Change something in the project, then run the below (alone) after to generate bin to uploade using mcumgr
# alias imgtool2.py="${HOME}/git/mcuboot_tmp/scripts/imgtool.py"
# west build -p -b nrf52840dk_nrf52840
# imgtool2.py sign -v 1.0.0 -H 0x200 -S 0x79e00 --align 8  --fix-sig-pubkey keys/pub.pem --vector-to-sign payload  --pad-header build/zephyr/app_to_sign.bin keys/data_to_sign.bin
# openssl dgst -sha256  -sigopt rsa_padding_mode:pss -sigopt rsa_pss_saltlen:32 -sigopt rsa_mgf1_md:sha256 -sign keys/priv.pem -out keys/openssl_sig.sig keys/data_to_sign.bin
# openssl base64 -in keys/openssl_sig.sig -out keys/openssl_sig_base64.sig
# imgtool2.py sign -v 1.0.0  -H 0x200 -S 0x79e00 --align 8 --fix-sig keys/openssl_sig_base64.sig --fix-sig-pubkey keys/pub.pem --pad-header  build/zephyr/app_to_sign.bin signed_app_image.bin
#
